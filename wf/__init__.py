...
def wtOpt_workflow(
    ...
    **optimize_wt** evaluates parameter combinations for spatial RNA-seq
    ...
    - `metadata.csv`: one-row manifest of the workflow inputs and parameter
      values, including run IDs, conditions, filtering thresholds, backend
      choice, and the optimization grid.
    ...
    - `spatial_coherence.csv`: per-parameter-set spatial coherence scores when
      a spatial neighbor graph can be built.
    ...
    - `all_umaps.html`: UMAP panels for each successful parameter set, colored
    ...
    - `svg_spatial.html`: spatial expression plots for the top spatially
      variable genes when SVG analysis completes successfully.
    ...
    - `deg_clusters_top<marker_top_n>.csv`: optional compact marker table with
      the top marker genes per cluster.
    - `figures/cluster_marker_heatmap_top<marker_top_n>.png`: optional
      marker-gene heatmap.
    ...
    - `_intermediates/preprocess/preprocessed.h5ad`: filtered, normalized,
      log-transformed, HVG-selected AnnData object used as input to
      optimization jobs.
    - `_intermediates/preprocess/figures/`: pre- and post-filtering QC violin
      plots from preprocessing.
